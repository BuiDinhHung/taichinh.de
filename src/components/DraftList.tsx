  ...
  if (articles.length === 0) {
    return (
      <div className="rounded-xl border border-dashed border-border-default p-12 text-center">
        <p className="text-base text-text-muted">Chưa có bài viết nào trong database.</p>
    ...
    <>
      ...
      <ul className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {articles.map((article) => {
          ...
          return (
            <li key={article.id}>
              <article className="flex h-full flex-col gap-3 rounded-xl border border-border-default bg-white p-5 transition-colors hover:border-brand-gold dark:bg-card dark:hover:border-primary">
